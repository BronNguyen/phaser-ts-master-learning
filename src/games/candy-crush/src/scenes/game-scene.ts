...
export class GameScene extends Phaser.Scene {
  ...
  highLight() {
    ...
    this.graphics.fillRect(0, 0, CONST.tileWidth, CONST.tileHeight);

    this.graphics.generateTexture("tile", CONST.tileWidth, CONST.tileHeight);
    const highlighted = this.add.image(CONST.tileWidth, CONST.tileHeight, "tile").setAlpha(0).setDepth(-1);

    // const selectedArea = new Phaser.Geom.Rectangle(0, 0, CONST.tileWidth, CONST.tileHeight);
    // const hitAreaCallback = Phaser.Geom.Rectangle.Contains;
    this.input.on("pointerdown", () => {
      highlighted.setAlpha(1);
      highlighted.setPosition(this.firstSelectedTile.x + CONST.tileWidth/2, this.firstSelectedTile.y + CONST.tileHeight/2);
    });

...
export class Mario extends Phaser.GameObjects.Sprite {
  ...
  private handleInput() {
    ...
    if (this.keys.get('FIRE').isDown && this.canShoot) {
      this.fire();
    }
  }

  private fire() {
    if(this.scene.time.now <= this.lastShoot) return;
    ...
    this.scene.events.emit('fire!', fireBall);
    this.lastShoot = this.scene.time.now + 700;
  }
  ...
  public growMario(): void {
    if (this.marioSize == 'big') {
      this.enhanceMario();
      // this.currentScene.registry.set();
    }
    this.marioSize = 'big';
